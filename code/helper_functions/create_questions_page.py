...
def CreateQuestions(aadhar_data, language):
    if language == 0:
...
<body onload="show_tab(0);">
    ...
    <div id="myModal" class="modal">
        ...
        with open("web/kannada/questions.html", "w") as f:
            f.write("""<!DOCTYPE html>
<html lang="en">
...
<body onload="show_tab(0);">
    <form id="regForm">""")
            ...
            while i > 0:
                ...
                f.write("""        <div class="tab animated fadeInTop" id="{}">""".format(n-i))
                f.write("\n")
                f.write("""            <h1 id="question" style="color: rgb(102, 252, 241); font-size: 3.125vw; text-align: center;"> {}"</h1>""".format(TranslateText("Please enter your " +  keys[i-1] + ".")))
                f.write("\n")
                if keys[i-1] == "Address":
                        f.write("""            <textarea class="txtbox use-keyboard-input" id="response" style="height: 12.5vw;">{}</textarea>""".format(aadhar_data['Address']))
                elif keys[i-1] == "Date of Birth":
            ...
            for i in range(len(data['root'])):
                ...
                f.write("""        <div class="tab animated fadeInTop" id="{}">""".format(n+i))
                f.write("\n")
                question_string = """            <h1 id="question" style="color: rgb(102, 252, 241); font-size: 3.125vw; text-align: center;">""" +  TranslateText(question) + "</h1>"
                f.write(question_string)
                f.write("\n")
                if widget == 'required':
                    if "enter" in question.lower() and "address" in question.lower():
