...
from helper_functions.create_questions_page import CreateQuestions, Cities
...
def TranslateText(text):
    ...
    translate_text = translator.translate(text, dest="kn")
    return translate_text.text    

@eel.expose
def ReadQuestion(text):
    speech = speak(text, "en")
    audio = AudioSegment.from_mp3(speech)
    audio = effects.speedup(audio, 1.2)
    play(audio) 

@eel.expose
def ReadQuestion_kannada(text): 
    speech = speak(text, "kn")
    audio = AudioSegment.from_mp3(speech)
    audio = effects.speedup(audio, 1.2)
    play(audio) 
...
@eel.expose
def ListenResponse_kannada():
    translator = Translator()  
    text =  Speech_to_Text("kn-IN")
    if (text):
        translate_text = translator.translate(text, dest="en")  
        print(translate_text.text)
        return translate_text.text.lower()
    else:
        print("No response")
        return None
    #else:
    ...
    while True:
        ...
        if(camera.aadhar_number==0):
            if(f%15==0):
                ...
                ReadQuestion_kannada("ಆಧಾರ್ ಪತ್ತೆಯಾಗಿಲ್ಲ.")
            yield frame
        else:
...
def aadhar_video():
    ...
    ReadQuestion_kannada("ದಯವಿಟ್ಟು ನಿಮ್ಮ ಆಧಾರ್ ಕಾರ್ಡ್ ಅನ್ನು ಕ್ಯಾಮರಾಕ್ಕೆ ಹಿಡಿದುಕೊಳ್ಳಿ.")
    ...
    for each in y:
        ...
        if(type(each)==str):
            ...
            if face == each:
                ...
                ReadQuestion_kannada("ಆಧಾರ್ ಮತ್ತು ಫೇಸ್ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ.")
                eel.restart()()
                break
...
def RetrieveData(id, num):
    data = json.loads(open(db_path).read())
    data_dict = data[id]
    CreateQuestions(data_dict, num)
    return num
